...
function Scene:load(async)
	...
	--ANALYTICS_HELPER.scene_load_start(self._name)
	SCENE_LOADER.load(self,function()
	--	ANALYTICS_HELPER.scene_load_end(self._name)
		self:load_done()
		self._state = SCENE_ENUMS.STATES.HIDE
		local load_time = socket.gettime() - time
		COMMON.i(string.format("%s loaded", self._name), TAG)
		COMMON.i(string.format("%s load time %s", self._name, load_time), TAG)
	--	ANALYTICS_HELPER.performance_scene_load_time(self._name, load_time)
	end)
...
--	ANALYTICS_HELPER.scene_unload(self._name)
	self:unload_done()
...
--	ANALYTICS_HELPER.scene_hide(self._name)
	self:hide_before()
...
function Scene:show()
	assert(self._state == SCENE_ENUMS.STATES.HIDE)
	--ANALYTICS_HELPER.scene_show(self._name)
